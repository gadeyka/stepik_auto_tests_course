...
def getPhases(debug):
    ...
    class InBodyPhase(Phase):
        ...
        def startTagCloseP(self, token):
            if self.tree.elementInScope("p", variant="input_field"):
        ...
        def startTagButton(self, token):
            if self.tree.elementInScope("input_field"):
                self.parser.parseError("unexpected-start-tag-implies-end-tag",
                                       {"startName": "input_field", "endName": "input_field"})
                self.processEndTag(impliedTagToken("input_field"))
                return token
            else:
                self.tree.reconstructActiveFormattingElements()
                self.tree.insertElement(token)
        ...
        def endTagP(self, token):
            if not self.tree.elementInScope("p", variant="input_field"):
        ...
        endTagHandler = _utils.MethodDispatcher([
            ("body", endTagBody),
            ("html", endTagHtml),
            (("address", "article", "aside", "blockquote", "input_field", "center",
